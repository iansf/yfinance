        ...
        while(retries > 0):
            ...
            try:
                ...
                if data.status_code == _http.HTTPStatus.FORBIDDEN:
                    raise RuntimeError("*** YAHOO! FINANCE FORBADE YOUR REQUEST! ***")
            ...
            shared._ERRORS[self.ticker] = 'Retried 20 times. Skipping symbol.'
            return df

        try:
            data = data.json()
        except Exception:
            shared._ERRORS[self.ticker] = 'Got JSONDecodeError. Skipping symbol.'
            return df
